...
Governing Decisions:
  DEC-2026-04-18-01: SPX price = spx_daily_close column always.
...
Swing Anchor Algorithm (DEC-2026-04-18-02):
  For BEARISH detection (prior swing HIGH anchor):
    ...
    3. The prior swing HIGH is the max of spx_daily_close in that pre-trough
  ...
  For BULLISH detection (prior swing LOW anchor):
    ...
    3. The prior swing LOW is the min of spx_daily_close in that post-peak
       slice.
...
def detect_divergence(
    ...
    _db_rows : list[tuple[str, float, float]] | None
        Injectable test override.  Each tuple is (date_str, spx_daily_close, mmth)
    ...
    window_df = pd.DataFrame(window_rows, columns=["date", "spx_daily_close", "mmth"])
    ...
    current_spx = current_rows[0][1]
    current_mmth = current_rows[0][2]

    # ------------------------------------------------------------------
    # 3. Swing anchor — BEARISH (prior swing HIGH)
    # ------------------------------------------------------------------
    trough_idx = window_df["spx_daily_close"].idxmin()
    ...
    if pre_trough_df.empty:
        ...
        prior_high_idx = window_df["spx_daily_close"].idxmax()
    else:
        prior_high_idx = pre_trough_df["spx_daily_close"].idxmax()

    prior_high_date = window_df.loc[prior_high_idx, "date"]
    prior_high_spx = window_df.loc[prior_high_idx, "spx_daily_close"]
    ...
    # ------------------------------------------------------------------
    peak_idx = window_df["spx_daily_close"].idxmax()
    peak_date = window_df.loc[peak_idx, "date"]

    post_peak_df = window_df[window_df["date"] >= peak_date]
    if post_peak_df.empty:
        prior_low_idx = window_df["spx_daily_close"].idxmin()
    else:
        prior_low_idx = post_peak_df["spx_daily_close"].idxmin()

    prior_low_date = window_df.loc[prior_low_idx, "date"]
    prior_low_spx = window_df.loc[prior_low_idx, "spx_daily_close"]
    ...
    if current_spx > prior_high_spx and current_mmth < prior_high_mmth:
        return (
            DivergenceResult.BEARISH,
            f"Bearish divergence: SPX {current_spx:.2f} > prior high "
    ...
    if current_spx < prior_low_spx and current_mmth > prior_low_mmth:
        return (
            DivergenceResult.BULLISH,
            f"Bullish divergence: SPX {current_spx:.2f} < prior low "
...
def _load_from_db(db_path: str, as_of_date: str) -> list[tuple]:
    ...
    Returns a combined list of (date, spx_daily_close, mmth) tuples covering
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT date, spx_daily_close, mmth
            FROM indicators
